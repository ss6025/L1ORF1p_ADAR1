...
class helper_funcs(object):
    ...
    def TP53_mut_df(self):
        ...
        # rename columns
        df.rename(columns={"SYMBOL" : "Gene", 
            "Allele.Freq.T" : "VAF"}, inplace=True)
        df = df[["Patient_ID", "Sample_ID", "Gene", "HGVSc", "HGVSp_Short", 
                 "Prob MT p53", "VAF", "ccf_Mcopies"]]
        df["Sample_ID"] = df["Sample_ID"].str.replace("T0", "T")
...
####### This is the step where NA values are added 
        ...
        df = df[~df["data"].isna()&~df["ORF1p Term"].isna()]
        df[["ADAR1 Term", "ORF1p Term", "IFN Term"]] = StandardScaler().fit_transform(df[["ADAR1 Term", "ORF1p Term", "IFN Term"]])
